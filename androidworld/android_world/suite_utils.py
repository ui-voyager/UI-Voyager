...
def _log_and_print(msg: str, *args: object) -> None:
  ...
  # Handle BlockingIOError with retry logic
  max_retries = 5
  ...
  for attempt in range(max_retries):
    try:
      ...
      if attempt < max_retries - 1:
        ...
        # Final attempt failed, try stderr as fallback
        try:
          import sys
          print(formatted, file=sys.stderr)
          sys.stderr.flush()
        except BlockingIOError:
          # If even stderr fails, log the issue and continue
          logging.error('Failed to print output due to BlockingIOError: %s', formatted[:100])
...
def _run_task(
  ...
  try:
    ...
    # Finalize task directory for agents that support it
    if agent and hasattr(agent, 'finalize_task'):
      agent_successful = task_successful if interaction_results.done else 0.0
      agent.finalize_task(agent_successful > 0.5)
