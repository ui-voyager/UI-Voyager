...
class AndroidWorldController(base_wrapper.BaseWrapper):
  ...
  def refresh_env(self):
    ...
    self._env = get_controller(
        ...
        emulator_path=self.env._coordinator._simulator._config.emulator_launcher.emulator_path,
        avd_name=self.env._coordinator._simulator._config.emulator_launcher.avd_name,
        android_sdk_root=self.env._coordinator._simulator._config.emulator_launcher.android_sdk_root,
        android_avd_home=self.env._coordinator._simulator._config.emulator_launcher.android_avd_home,
        adb_server_port=self.env._coordinator._simulator._config.adb_controller.adb_server_port,
    ).env
...
def get_controller(
    ...
    android_sdk_root: str = '/root/android/',
  ...
  Args:
    ...
    avd_name: Name of the AVD to use.
    android_sdk_root: Root directory of the Android SDK.
    android_avd_home: Home directory for AVD files.
    adb_server_port: Port for the ADB server (default 5037).

  Returns:
    AndroidWorldController instance.
  """

  # print('~~~~~~~~~~~~~~~~~~~ android_env launch ~~~~~~~~~~~~~~~~~~~')
  # print(adb_path)
