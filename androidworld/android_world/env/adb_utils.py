...
def type_text(
  ...
  Returns:
    The adb response. Returns the first failed response if any word fails,
    otherwise returns the last successful response.
  """
  words = list(_split_words_and_newlines(text))
  last_response = None
  
  for word in words:
    if word == '\n':
      logging.info('Found \\n, pressing enter button.')
      last_response = press_enter_button(env)
      if last_response.status != adb_pb2.AdbResponse.Status.OK:
  ...
  # If text is empty, return a successful empty response
  if last_response is None:
    last_response = adb_pb2.AdbResponse(status=adb_pb2.AdbResponse.Status.OK)
...
def launch_app(
  ...
  if app_name in _DEFAULT_URIS:
    return _launch_default_app(app_name, env)

  activity = get_adb_activity(app_name)
  if activity is None:
    #  If the app name is not in the mapping, assume it is a package name.
    # Note: app_name may contain spaces, needs to be quoted
    response = issue_generic_request(
        ['shell', 'monkey', '-p', f'"{app_name}"', '1'], env, timeout_sec=5
    )
    logging.info('Launching app by package name, response: %r', response)
    return response
  return start_activity(activity, extra_args=[], env=env, timeout_sec=5)
