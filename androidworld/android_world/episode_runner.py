...
def run_episode(
  ...
  # Build log prefix with worker_id and task_name info
  log_prefix = ''
  ...
  for step_n in range(max_n_steps):
    result = agent.step(goal, task_name)
    print_fn(f'{log_prefix} Completed step {step_n + 1:d}.')
    ...
    if termination_fn(agent.env):
      print_fn(f'{log_prefix} Environment ends episode.')
      return EpisodeResult(
          done=True,
          step_data=_transpose_lod_to_dol(output),
      )
    elif result.done:
      print_fn(f'{log_prefix} Agent indicates task is done.')
      return EpisodeResult(
          done=result.done,
          step_data=_transpose_lod_to_dol(output),
      )
  print_fn(
      termcolor.colored(
          f'{log_prefix} Agent did not indicate task is done. Reached max number of steps.',
          'red',
      )
  )
  return EpisodeResult(
