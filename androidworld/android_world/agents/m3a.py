...
class M3A(base_agent.EnvironmentInteractingAgent):
  ...
  def __init__(
      ...
      base_screenshot_dir: str = '/apdcephfs_tj4/share_303922427/altmanyang/logs/m3a_debug_screenshots',
  ):
  ...
  def finalize_task(self, success: bool) -> None:
    ...
    if self.current_task_dir and '_temp' in self.current_task_dir:
      ...
      try:
        ...
        logging.info('Renamed task directory: %s -> %s', old_dir, new_dir)
        self.current_task_dir = new_dir
      except Exception as e:
        logging.error('Failed to rename task directory: %s', str(e))
  ...
  def step(self, goal: str, task_name: Optional[str] = None) -> base_agent.AgentInteractionResult:
    ...
    if self.timestamp_dir is None:
      ...
      logging.info('Created timestamp directory: %s', self.timestamp_dir)
    ...
    if self.current_task_dir is None and task_name:
      ...
      logging.info('Created task directory: %s', self.current_task_dir)
    ...
    print(str(len(self.history) + 1), action_output)
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    # Save action output to text file
    if self.current_task_dir:
      step_num = len(self.history) + 1
      ...
      with open(response_path, 'w', encoding='utf-8') as f:
        f.write(f'<goal>\n{goal}\n</goal>\n\n')
        f.write(action_output)
      logging.info('Saved response to: %s', response_path)
