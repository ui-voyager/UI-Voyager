...
def _get_unique_tmp_directory() -> str:
  """Create a unique temporary directory to avoid file contention between multiple emulators.
  
  Returns:
    Unique temporary directory path
  """
  unique_dir = os.path.join(_TMP, f"aw_{uuid.uuid4().hex[:8]}")
  os.makedirs(unique_dir, exist_ok=True)
  return unique_dir


def _cleanup_tmp_directory(tmp_dir: str) -> None:
  """Clean up a temporary directory.
  
  Args:
    tmp_dir: Path of the temporary directory to clean up
  """
  try:
    if tmp_dir.startswith(_TMP) and "aw_" in tmp_dir:
      shutil.rmtree(tmp_dir, ignore_errors=True)
  except Exception as e:
    logging.warning("Failed to cleanup tmp directory %s: %s", tmp_dir, e)
...
def write_to_gallery(
  ...
  # Use a separate tmp directory to avoid file contention between multiple emulators
  tmp_dir = _get_unique_tmp_directory()
  temp_storage_location = file_utils.convert_to_posix_path(tmp_dir, file_name)
  ...
  try:
    ...
    _cleanup_tmp_directory(tmp_dir)
...
def write_video_file_to_device(
  ...
  # Use a separate tmp directory to avoid file contention between multiple emulators
  tmp_dir = _get_unique_tmp_directory()
  temp_file_path = file_utils.convert_to_posix_path(tmp_dir, file_name)
...
def write_mp3_file_to_device(
  ...
  # Use a separate tmp directory to avoid file contention between multiple emulators
  tmp_dir = _get_unique_tmp_directory()
  local = file_utils.convert_to_posix_path(tmp_dir, os.path.basename(remote_path))
  _create_test_mp3(
      local,
      artist=artist,
      title=title,
