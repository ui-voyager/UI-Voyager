...
class TaskManager:
  ...
  def stop(self) -> None:
    ...
    try:
      ...
      logging.exception('Failed to stop dumpsys thread. Continuing.')
  ...
  def _stop_dumpsys_thread(self):
    """停止 dumpsys 线程，释放 ThreadPoolExecutor 资源."""
    if self._dumpsys_thread is not None:
      # DumpsysThread 使用 ThreadPoolExecutor，需要等待当前任务完成
      if hasattr(self._dumpsys_thread, '_latest_check') and self._dumpsys_thread._latest_check is not None:
        try:
          # 等待当前检查完成，最多等待 3 秒
          self._dumpsys_thread._latest_check.result(timeout=3.0)
        except Exception:
          pass  # 忽略超时或其他错误
      self._dumpsys_thread = None
